...
from official.nlp.bert import tokenization_morp_utagger_ver_old
from official.nlp.data import create_span_ej_tagging_finetuning_data


# test 할 때는 'train_and_eval'=>predict으로 변경
flags.DEFINE_enum(
    'mode', 'predict', ['train_and_eval', 'export_only', 'predict'],
...
flags.DEFINE_string('train_data_path', r'C:\Users\klplab\Desktop\ubert\train_span_final_no_nv',
                    'Path to training data for BERT classifier.')

flags.DEFINE_string('meta_data_path', r'C:\Users\klplab\Desktop\ubert\express_span_final_no_nv.json',
                    'Path to training data for BERT classifier.')
# validation
# flags.DEFINE_string('eval_data_path', r'C:\Users\klplab\Desktop\ubert\validation_span_final_no_nv',
#                     'Path to evaluation data for BERT classifier.')

# test
flags.DEFINE_string('eval_data_path', r'C:\Users\klplab\Desktop\ubert\test_span_final_no_nv',
                    'Path to evaluation data for BERT classifier.')

flags.DEFINE_string('eval_file_path', r'C:\Users\klplab\Desktop\ubert\test_span_no_ej.txt',
                    'Path to evaluation data for BERT classifier.')
...
def get_dataset_fn(input_file_pattern,
  ...
  def _dataset_fn(ctx=None):
    ...
    dataset = input_pipeline.create_span_ud_dataset(
...
def get_predictions_and_labels(strategy,
  ...
  def test_step(iterator):
    ...
    def _test_step_fn(inputs):
      ...
      if not is_regression:
        ...
        return probabilities1, probabilities2, labels
      else:
        return logits, labels

    start, end, labels = strategy.run(_test_step_fn, args=(next(iterator),))
  ...
  def _run_evaluation(test_iterator):
    ...
    preds_s, preds_e, golds_s, golds_e = list(), list(), list(), list()
    try:
      with tf.experimental.async_scope():
        while True:
          probabilities_s, probabilities_e, labels = test_step(test_iterator)
          for cur_probs_s, cur_probs_e, cur_labels_s, cur_labels_e in zip(probabilities_s, probabilities_e, labels['start_label'], labels['end_label']):
            if return_probs:
    ...
    return preds_s, preds_e, golds_s, golds_e

  test_iter = iter(strategy.distribute_datasets_from_function(eval_input_fn))
  predictions_s, predictions_e, labels_s, labels_e = _run_evaluation(test_iter)

  return predictions_s, predictions_e, labels_s, labels_e
...
def get_loss_fn(num_classes):
  ...
  def classification_loss_fn(start_label, end_label, start_logits, end_logits, attention_mask):
    """Classification loss."""
    ...
    return tf.reduce_mean((active_start_loss +active_end_loss) / 2)
...
def custom_metric_fn(start_label, end_label, start_pred, end_pred, attention_mask) :
  active_loss = attention_mask==1
  ...
  return tf.reduce_mean(start_numeric), tf.reduce_mean(end_numeric)
...
def main(_) :
    ...
    FLAGS.model_dir = 'C:\\Users\\klplab\\Desktop\\ubert\\express_span_ej_10_15_final4\\'
    ...
    if FLAGS.mode == 'predict':
        with strategy.scope():
            ...
            preds_s, preds_e, golds_s, golds_e = get_predictions_and_labels(
                strategy,
                classifier_model,
                eval_input_fn,
        ...
        eval_datas = create_span_ej_tagging_finetuning_data.read_Seq2Seq(input_file=FLAGS.eval_file_path, 
                        read_function=create_span_ej_tagging_finetuning_data.read_tsv())
        tokenizer = tokenization_morp_utagger_ver_old.KoreanMorpsTokenizer(
        ...
        with tf.io.gfile.GFile(output_predict_file, 'w') as writer:
            ...
            labels_dic = create_span_ej_tagging_finetuning_data.get_labels()
            # reverse_dic
            v_k_labels_dic = dict()
            ...
            for idx, (probabilities_s, probabilities_e, goldslabel_s, goldslabel_e) in enumerate(zip(preds_s,preds_e, golds_s, golds_e)):
              result_labels_start = convert_logits_to_labels(probabilities_s)
              result_labels_end = convert_logits_to_labels(probabilities_e)
              real_labels_start = goldslabel_s
              real_labels_end = goldslabel_e
              eval_data = eval_datas[idx]
              eval_tokens, morp_nums = tokenizer.tokenize_and_morp_number(eval_data.text)
              eval_len = len(eval_tokens)

              # morp_nums 를 통해 정답 축소, 정답의 처음과 끝은 제거 CLS, SEP 토큰임
              fixed_answer_start, fixed_answer_end, fixed_real_start, fixed_real_end = dict(), dict(), dict(), dict()
              result_labels_start = result_labels_start[1: eval_len + 1]
              result_labels_end = result_labels_end[1: eval_len + 1]
              real_labels_start = real_labels_start[1: eval_len + 1]
              real_labels_end = real_labels_end[1: eval_len + 1]

              if len(morp_nums)>512:
                morp_nums = morp_nums[:512]
              
              for token_idx, num in enumerate(morp_nums) :
                if num not in fixed_answer_start :
                  fixed_answer_start[num] = result_labels_start[token_idx]
                if num not in fixed_answer_end :
                  fixed_answer_end[num] = result_labels_end[token_idx]
                if num not in fixed_real_start :
                  fixed_real_start[num] = real_labels_start[token_idx]
                if num not in fixed_real_end :
                  fixed_real_end[num] = real_labels_end[token_idx]

              result_fixed_tag_start = [value for _, value in fixed_answer_start.items()]
              result_fixed_tag_end = [value for _, value in fixed_answer_end.items()]
              result_real_tag_start = [value for _, value in fixed_real_start.items()]
              result_real_tag_end = [value for _, value in fixed_real_end.items()]

              R = bert_extract_item(result_fixed_tag_start, result_fixed_tag_end)
              T = bert_extract_item(result_real_tag_start, result_real_tag_end)
