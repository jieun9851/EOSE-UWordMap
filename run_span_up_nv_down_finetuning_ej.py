...
from official.nlp.data import create_span_ej_finetuning_data
...
flags.DEFINE_string('eval_file_path', r'C:\Users\klplab\Desktop\ubert\test_span_ud_nv_ej_final_zzin.txt',
                    'Path to evaluation data for BERT classifier.')
...
def get_dataset_fn(input_file_pattern,
  ...
  def _dataset_fn(ctx=None):
    ...
    dataset = input_pipeline.create_span_ud_nv_dataset(
...
def main(_) :
    ...
    FLAGS.model_dir = 'C:\\Users\\klplab\\Desktop\\ubert\\express_span_ej_11_30_final2\\'
    ...
    if FLAGS.mode == 'predict':
        ...
        eval_datas = create_span_ej_finetuning_data.read_Seq2Seq(input_file=FLAGS.eval_file_path, 
                        read_function=create_span_ej_finetuning_data.read_tsv())
        ...
        with tf.io.gfile.GFile(output_predict_file, 'w') as writer:
            ...
            labels_dic = create_span_ej_finetuning_data.get_labels()
            # reverse_dic
            v_k_labels_dic = dict()
            ...
            for idx, (probabilities_s, probabilities_e) in enumerate(zip(preds_s,preds_e)):
              ...
              # for token_idx, num in enumerate(ej_nums) :
              #   if num not in fixed_answer_start :
              #     fixed_answer_start[num] = result_labels_start[token_idx]
              #   elif num in fixed_answer_start and result_labels_start[token_idx] ==1:
              #     fixed_answer_start[num] = result_labels_start[token_idx]
              #   if num not in fixed_answer_end :
              #     fixed_answer_end[num] = result_labels_end[token_idx]
              #   elif num in fixed_answer_end and result_labels_end[token_idx] == 1:
              #     fixed_answer_end[num] = result_labels_end[token_idx]
